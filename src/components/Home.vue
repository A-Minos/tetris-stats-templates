<script lang="ts" setup>
import type { BindingStatus } from '@/core/enums'
import { useRouter } from 'vue-router'
import { data } from '@/core/template'

const router = useRouter()

const redirect_binding = () => {
	data.value = {
		type: 'binding',
		data: {
			game_type: prompt('游戏类型', 'tetr.io')!,
			status: prompt('类型 (error, success, unverified, unknown, unlink)', 'unknown')! as BindingStatus,
			user_name: prompt('用户名', 'creeperqwq')!,
			user_avatar: prompt('用户头像', 'data:image/svg+xml;base64,')!,
			bot_name: prompt('bot 名', 'linmo')!,
			bot_avatar: prompt('bot 头像', 'data:image/svg+xml;base64,')!,
			command: prompt('指令', 'fkosk')!
		}
	}

	router.push({
		path: '/binding'
	})
}
</script>

<template>
	<div class="flex gap-1 p-2">
		<button @click="redirect_binding">测试 Binding</button>
	</div>
</template>